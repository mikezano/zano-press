...
Notice that the overflow is cut off leaving us with shape that can be colored in to match the background it is on. Doing that on the marquee will leave an inward looking swoop. Following that pattern, for the other corners we would have (x,y) values of:
...
- The middle light is selected by `3n+2` and a further delay `0.4s`.
