<template>
	<div class="posts" v-if="posts.length">
		<div class="post" v-for="post in posts">
			...
			<Badge v-bind:text="post.frontmatter.tag" type="tag"/>
...
export default {
	...
	computed: {
		posts() {
			...
			if (this.all) regex = `(?=.*html)`;
			else regex = `(${currentPage})(?=.*html)`;

			let posts = this.$site.pages
				.filter(x => {
					return (
						x.path.match(new RegExp(regex)) && x.frontmatter.date
					);
				})
				.sort((a, b) => {
...
.post {
	...
	&__title:hover::before {
		width: 100%;
		transition: all 0.4s cubic-bezier(0, -0.02, 0.06, 1.05);
	}
	...
	&__title::before {
		...
		height: 100%;
		content: '';
		border-bottom: 0.2rem solid hsla(206, 100%, 39%, 1);
	}
